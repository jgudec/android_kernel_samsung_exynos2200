...
#define info(fmt, args...) \
	warn_common("[HM3]", fmt, ##args)

#define SENSOR_HM3_FRAME_COUNT_LOG_MAX 8

#define is_sensor_write16(client, addr, val) \
	is_sensor_write16(client, addr, val); \
	do { \
		if (sensor_hm3_frame_count < SENSOR_HM3_FRAME_COUNT_LOG_MAX) { \
			info("[%s] 0x%04X=0x%04X\n", __func__, addr, val); \
		} \
	} while(0)

#define is_sensor_write8(client, addr, val) \
	is_sensor_write8(client, addr, val); \
	do { \
		if (sensor_hm3_frame_count < SENSOR_HM3_FRAME_COUNT_LOG_MAX) { \
