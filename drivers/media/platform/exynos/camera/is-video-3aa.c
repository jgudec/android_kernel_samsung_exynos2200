	...
	for (i = 0; i < ctrls->count; i++) {
		...
		case V4L2_CID_SENSOR_SET_CAPTURE_INTENT_INFO:
		{
			...
			if (info.captureIntent == AA_CAPTURE_INTENT_STILL_CAPTURE_OIS_MULTI
				...
				|| info.captureIntent == AA_CAPTURE_INTENT_STILL_CAPTURE_EXECUTOR_NIGHT_SHOT) {
			...
			mginfo("s_ext_ctrl SET_CAPTURE_INTENT_INFO, intent(%d) count(%d) captureEV(%d) captureIso(%d) captureAeExtraMode(%d) remainIntentCount(%d)\n",
				head, head, info.captureIntent, info.captureCount, info.captureEV, info.captureIso,
				info.captureAeExtraMode, head->remainIntentCount);
			break;
		}
		default:
			ctrl.id = ext_ctrl->id;
