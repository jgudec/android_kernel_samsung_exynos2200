...
static unsigned int dpu_fence_event_log_max = 1024;
static unsigned int dpu_fence_event_print_max = 64;
...
module_param_named(fence_event_log_max, dpu_fence_event_log_max, uint, 0);
module_param_named(fence_event_print_max, dpu_fence_event_print_max, uint, 0600);
...
MODULE_PARM_DESC(fence_event_log_max, "entry count of fence event log buffer array");
MODULE_PARM_DESC(fence_event_print_max, "print entry count of fence event log buffer");

/* If event are happened continuously, then ignore */
static bool
dpu_event_ignore(enum dpu_event_type type, struct dpu_debug *d)
{
	u32 event_log_cnt = d->event_log_cnt;
	int latest = atomic_read(&d->event_log_idx) % event_log_cnt;
	int idx, offset;
	...
	for (offset = 0; offset < DPU_EVENT_KEEP_CNT; ++offset) {
		idx = (latest + event_log_cnt - offset) % event_log_cnt;
...
/* If event are happened continuously, then ignore */
static bool
dpu_fence_event_ignore(enum dpu_fence_event_type type, struct dpu_debug *d)
{
	u32 fence_event_log_cnt = d->fence_event_log_cnt;
	int latest = atomic_read(&d->fence_event_log_idx) % fence_event_log_cnt;
	int idx, offset;

	if (IS_ERR_OR_NULL(d->fence_event_log))
		return true;

	for (offset = 0; offset < DPU_EVENT_KEEP_CNT; ++offset) {
		idx = (latest + fence_event_log_cnt - offset) % fence_event_log_cnt;
		if (type != d->fence_event_log[idx].type)
			return false;
	}
		...
		void *priv)
{
	...
	idx = atomic_inc_return(&d->event_log_idx) % d->event_log_cnt;
	log = &d->event_log[idx];
	...
	case DPU_EVT_DMA_RECOVERY:
		...
		d->err_event_cnt++;
		break;
	case DPU_EVT_DMA_ERR:
	case DPU_EVT_DPP_CFGERR:
	case DPU_EVT_RESOURCE_CONFLICT:
	case DPU_EVT_DECON_TIMEOUT:
	case DPU_EVT_FRAMESTART_TIMEOUT:
	case DPU_EVT_VBLANK_TIMEOUT:
		d->err_event_cnt++;
		break;
		...
		struct exynos_drm_crtc *exynos_crtc, void *priv)
{
	...
	switch (type) {
	case DPU_FENCE_SIGNAL_CRTC_OUT_FENCE:
		/*
		 * If the same event occurs DPU_EVENT_KEEP_CNT times
		 * continuously, it will be skipped.
		 */
		if (dpu_fence_event_ignore(type, &exynos_crtc->d))
			return;
		break;
	default:
		break;
	...
	spin_lock_irqsave(&d->fence_event_lock, flags);
	idx = atomic_inc_return(&d->fence_event_log_idx) % d->fence_event_log_cnt;
	log = &d->fence_event_log[idx];
	spin_unlock_irqrestore(&d->fence_event_lock, flags);

	log->time = ktime_get();
	log->type = type;

	switch (type) {
	case DPU_FENCE_ARM_CRTC_OUT_FENCE:
	case DPU_FENCE_WAIT_PLANE_IN_FENCE:
		fence = (struct dma_fence *)priv;
		strlcpy(log->fence_info.name, fence->ops->get_driver_name(fence),
				MAX_FENCE_DRV_NAME);
		strlcpy(log->fence_info.timeline, fence->ops->get_timeline_name(fence),
				MAX_FENCE_DRV_NAME);
		log->fence_info.context = fence->context;
		log->fence_info.seqno = fence->seqno;
		log->fence_info.flags = fence->flags;
		break;
	case DPU_FENCE_TIMEOUT_IN_FENCE:
	case DPU_FENCE_ELAPSED_IN_FENCE:
		log->fence_info.in_fence_wait_ms = *(s64 *)priv;
		break;
	default:
		break;
	}
		...
		int index, struct drm_printer *p)
{
	...
	len += scnprintf(buf + len, sizeof(buf) - len,
			"ROT[%d] COMP[%d] HDR[%d] ", win->is_rot, win->is_comp, win->is_hdr);
...
const char *get_event_name(enum dpu_event_type type)
{
	static const char events[][MAX_EVENT_NAME] = {
		...
		"DPP_FRAMEDONE",		"DPP_CONFIG",
		"DMA_RECOVERY",			"DMA_ERR",
		...
		"RESOURCE_CONFLICT",		"DECON_TIMEOUT",
		"VBLANK_TIMEOUT",
		"TUI_ENTER",			"TUI_EXIT",
...
const char *get_fence_event_name(enum dpu_fence_event_type type)
{
	static const char events[][MAX_EVENT_NAME] = {
		"NONE",
		"ARM_CRTC_OUT_FENCE",	"SIGNAL_CRTC_OUT_FENCE",
		"WAIT_PLANE_IN_FENCE",	"TIMEOUT_PLANE_IN_FENCE",
		"ELAPSED_PLANE_IN_FENCE",

	};

	if (type >= DPU_FENCE_MAX)
		return NULL;
...
#endif
{
	...
	struct dpu_log *log;
	int latest = idx % d->event_log_cnt;
	...
	if (dpu_event_print_max > d->event_log_cnt)
		dpu_event_print_max = d->event_log_cnt;

	if (idx < dpu_event_print_max)
		idx = 0;
	else
		idx = (idx - dpu_event_print_max) % d->event_log_cnt;

	prev_ktime = ktime_set(0, 0);
	do {
		if (++idx >= d->event_log_cnt)
			idx = 0;
...
static void
DPU_FENCE_EVENT_SHOW(const struct exynos_drm_crtc *exynos_crtc, struct drm_printer *p)
{
	const struct dpu_debug *d = &exynos_crtc->d;
	int idx = atomic_read(&d->fence_event_log_idx);
	struct dpu_fence_log *log;
	int latest = idx % d->fence_event_log_cnt;
	struct timespec64 ts;
	ktime_t prev_ktime;
	char buf[LOG_BUF_SIZE];
	int len;
	...
	/* Seek a oldest from current index */
	if (dpu_fence_event_print_max > d->fence_event_log_cnt)
		dpu_fence_event_print_max = d->fence_event_log_cnt;

	if (idx < dpu_fence_event_print_max)
		idx = 0;
	else
		idx = (idx - dpu_fence_event_print_max) % d->fence_event_log_cnt;

	prev_ktime = ktime_set(0, 0);
	do {
		if (++idx >= d->fence_event_log_cnt)
			idx = 0;
		...
		len += scnprintf(buf + len, sizeof(buf) - len,  "%25s",
				get_fence_event_name(log->type));

		switch (log->type) {
		case DPU_FENCE_ARM_CRTC_OUT_FENCE:
		case DPU_FENCE_WAIT_PLANE_IN_FENCE:
			scnprintf(buf + len, sizeof(buf) - len,
					"\tname:%s timeline:%s ctx:%d seqno:%d flags:%#x",
					log->fence_info.name, log->fence_info.timeline,
					log->fence_info.context, log->fence_info.seqno,
					log->fence_info.flags);
			break;
		case DPU_FENCE_TIMEOUT_IN_FENCE:
		case DPU_FENCE_ELAPSED_IN_FENCE:
			scnprintf(buf + len, sizeof(buf) - len,
					"\tin_fence_wait:%lldmsec",
					log->fence_info.in_fence_wait_ms);
			break;
		default:
			break;
		}
...
static int dpu_debug_fence_event_show(struct seq_file *s, void *unused)
{
	struct exynos_drm_crtc *exynos_crtc = s->private;
	struct drm_printer p = drm_seq_file_printer(s);

	DPU_FENCE_EVENT_SHOW(exynos_crtc, &p);
	return 0;
}

static int dpu_debug_fence_event_open(struct inode *inode, struct file *file)
{
	return single_open(file, dpu_debug_fence_event_show, inode->i_private);
}

static const struct file_operations dpu_fence_event_fops = {
	.open = dpu_debug_fence_event_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = seq_release,
};
...
int dpu_init_debug(struct exynos_drm_crtc *exynos_crtc)
{
	...
	exynos_crtc->d.fence_event_log = NULL;
	event_cnt = dpu_fence_event_log_max;

	for (i = 0; i < DPU_FENCE_EVENT_LOG_RETRY; ++i) {
		event_cnt = event_cnt >> i;
		exynos_crtc->d.fence_event_log =
			vzalloc(sizeof(struct dpu_fence_log) * event_cnt);
		if (IS_ERR_OR_NULL(exynos_crtc->d.fence_event_log)) {
			DRM_WARN("failed to alloc fence event log buf[%d]. retry\n",
					event_cnt);
			continue;
		}

		DRM_INFO("#%d fence event log buffers are allocated\n", event_cnt);
		break;
	}
	spin_lock_init(&exynos_crtc->d.fence_event_lock);
	exynos_crtc->d.fence_event_log_cnt = event_cnt;
	atomic_set(&exynos_crtc->d.fence_event_log_idx, -1);
	...
	debugfs_create_file("fence_event", 0444, crtc->debugfs_entry, exynos_crtc,
			&dpu_fence_event_fops);
