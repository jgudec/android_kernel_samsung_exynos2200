...
#define ALIVE_RTC_BASE		0x15920000
#define CURTICCNT0		0x90
#endif
	...
	case IOCTL_TRIGGER_CP_CRASH:
	{
		...
		case PROTOCOL_SIPC:
			...
			mif_err("%s: IOCTL_TRIGGER_CP_CRASH (%lu)\n",
					iod->name, arg);
#if IS_ENABLED(CONFIG_SOC_S5E9925)
			if (ld->crash_reason.type == CRASH_REASON_RIL_MNR) {
				void __iomem *rtc_addr = devm_ioremap(mc->dev,
						ALIVE_RTC_BASE + CURTICCNT0, SZ_32);

				if (!rtc_addr)
					mif_err("Err: failed to ioremap RTC value register!\n");
				else
					mif_info("RTC before s2d: %d\n", __raw_readl(rtc_addr));
