...
{
	if (!fcd_available) {
		pr_info("[SSP] [FACTORY] %s: fcd not available\n", __func__);
		return -1;
	} else {
		pr_info("[SSP] [FACTORY] %s\n", __func__);
...
{
	struct ssp_data *data = container_of(work, struct ssp_data, fcd_notifier_work.work);
#if IS_ENABLED(CONFIG_FLIP_COVER_DETECTOR_NOTIFIER)
	fcd_notifier_call_chain(data->fcd_data.nfc_cover_status, NULL);
#endif
}
				      ...
				      struct device_attribute *attr, const char *buf, size_t size)
{
	...
	schedule_delayed_work(&data->fcd_notifier_work, msecs_to_jiffies(0));
