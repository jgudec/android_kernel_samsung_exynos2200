...
int open_pressure_sw_offset_file(struct ssp_data *data)
{
	...
	int sw_offset = 0;
	struct path path;
	char path_name[255] = SW_OFFSET_FILE_PATH;
	...
	ret = file_manager_read(data, SW_OFFSET_FILE_PATH, (char *)&sw_offset, sizeof(sw_offset), 0);
	if (ret < 0) {
		pr_err("[SSP]: %s - Can't read  sw offset file from file (%d)\n", __func__, ret);
		return ret;
	}
	data->sw_offset = sw_offset;
	pr_err("[SSP]: %s  sw offset file %d\n", __func__, sw_offset);
					 ...
					 size_t size)
{
	...
	ret = file_manager_write(data, SW_OFFSET_FILE_PATH, (char *)&sw_offset,  sizeof(sw_offset), 0);
	if (ret != sizeof(sw_offset)) {
		pr_err("[SSP]: %s - Can't write sw offset to file\n", __func__);
		ret = -EIO;
	}
