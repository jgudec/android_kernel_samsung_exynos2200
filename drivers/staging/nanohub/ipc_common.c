...
retry:
	...
	if ((uint64_t)buf < (uint64_t)ipc_data ||
	    (uint64_t)buf > ((uint64_t)ipc_data + (uint64_t)cipc_get_size(reg))) {
		CIPC_PRINT("out of cipc area scope : %p\n", buf);
		CIPC_PRINT("buf : %x : area start : %x : area end : %x\n",
			   (uint64_t)buf, (uint64_t)ipc_data,
